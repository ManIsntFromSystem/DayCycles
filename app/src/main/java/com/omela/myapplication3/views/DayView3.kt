...
class DayView3(context: Context) : View(context) {
    ...
    private var radialScale = 0f
    private var radialScaleDirection = 0f
    ...
    private fun drawCelestialBody(canvas: Canvas) {
        ...
        val scale = incScale()
        val mMatrix = Matrix().apply { setScale(scale, scale, cbX, cbY) }
    ...
    private fun incScale(): Float {
        radialScale += progress * radialScaleDirection
        if (radialScale <= 0.2f) {
            radialScaleDirection = 1f
            radialScale = 0.2f
